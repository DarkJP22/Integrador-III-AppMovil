...
import { alertController, InfiniteScrollCustomEvent } from '@ionic/core';
...
const { allPostsQuery, deleteSocialPost } = useMedicSocialPosts(medicId);
...
const socialPosts = computed(() => {
    return allPostsQuery.data.value?.pages.flatMap((page) => page.data) ?? [];
});
...
const loadMoreData = async (ev: InfiniteScrollCustomEvent) => {
    ...
    if (allPostsQuery.hasNextPage.value) {
        await allPostsQuery.fetchNextPage();
    }
...
<template>
    <div class="gallery-container">
        <div v-if="socialPosts.length === 0" class="no-posts-message text-center pt-4">
            <p>No hay publicaciones</p>
        </div>
        <div class="gallery-grid grid grid-cols-3 gap-1">
            <div v-for="(post) in socialPosts" :key="post.id" class="gallery-item aspect-square">

                <CldImage v-if="post.type === 'image'" :public-id="post.public_id" @click="openMedia(post)" />
                <div class="relative w-full h-full" @click="openMedia(post)" v-if="post.type === 'video'">
                    <div class="absolute inset-0 z-10 w-full h-full"></div>
                    <CldVideo :public-id="post.public_id" :controls="false" classes="w-full h-full object-cover object-center" />
                </div>

            </div>
        </div>
        <ion-infinite-scroll @ionInfinite="loadMoreData($event)" id="infinite-scroll" v-if="allPostsQuery.hasNextPage">
  ...
  <div v-if="showMediaModal" class="modal">
    ...
    <div class="modal-content">
      ...
      <CldImage :public-id="socialImage.public_id" v-if="socialImage?.type === 'image'" className="w-full h-full max-h-[80vh] object-contain object-center" />
