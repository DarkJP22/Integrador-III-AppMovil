  ...
  <form action="#" @submit.prevent="onSave()">
    <ion-card>
      ...
      <ion-item>
       ...
       <ion-item v-if="teleconsultationFee">
        <ion-input label="Costo de la teleconsulta" labelPlacement="floating" type="text" :value="teleconsultationFee" readonly>
          <div slot="start">₡</div>
        </ion-input>
      </ion-item>
      <ion-item v-if="teleconsultationFee">
        <div class="ion-padding">
          <p>Para reservar la teleconsulta, suba el comprobante de pago.</p>
          <input type="file" ref="fileInput" @change="onChangeFileUpload" class="hidden" accept="image/*,.pdf" />

          <ion-button color="tertiary" expand="block" shape="round" @click="($refs.fileInput as HTMLInputElement).click()" v-show="!isLoading">
            Subir Comprobante
          </ion-button>
          <ul class="files-ul">
...
import useAppointmentConsultations from "../composables/useAppointmentConsultations";
...
//import useAuth from "@/modules/auth/composables/useAuth";
const emit = defineEmits(['appointmentConsultationCreated']);
...
const { isLoading, errors, createAppointmentConsultation, appointmentConsultations } =
  useAppointmentConsultations();
...
const { maxUploadSizeInMb } = useGlobalSettings();
...
const onSave = async () => {
  const { ok, message, appointmentConsultation } = await createAppointmentConsultation(form.value, files.value);
  if (!ok) alertMessage("Error", message);
  else {
    //toastMessage("Cita Creada correctamente");
    alertMessage("Teleconsulta Enviada", "Teleconsulta Creada correctamente");
    emit("appointmentConsultationCreated", appointmentConsultation);
  }
};

const isReserved = (startSchedule: string, endSchedule: string) => {
  for (let j = 0; j < appointmentConsultations.value.length; j++) {
    if (
      appointmentConsultations.value[j].end > startSchedule &&
      appointmentConsultations.value[j].start < endSchedule
    ) {
      return true;
    }
  }
...
const generateAvailableDates = async () => {
  ...
  const scheduleDates = [...new Set(schedules.value.filter(sch => sch.title.toLowerCase().includes('teleconsulta')).map((item: any) => {
...
const generateAvailableHours = async () => {
  ...
  //await getAppointmentConsultations({ medicId: +props.medicId, officeId: +props.officeId, date1: form.value.date, date2: form.value.date });
  ...
  const schedulesOfDay = schedules.value.filter(
    (sch: any) => parseDate(sch.date) == parseDate(form.value.date) && sch.title.toLowerCase().includes('teleconsulta')
  );
...
const teleconsultationFee = computed(() => {
  if (!medic.value.offices) return 0;
  return medic.value?.offices.find((office: any) => office.id === props.officeId)?.settings?.teleconsultation_fee ?? 0;
});
...
const onChangeFileUpload = () => {
  ...
  if (files.value[0].size > maxSizeInBytes) { // 10mb
